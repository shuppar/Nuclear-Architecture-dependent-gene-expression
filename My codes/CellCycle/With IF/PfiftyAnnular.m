...
bw1 = imfill(bw, 'holes');
bw2 = imopen(bw1, strel('disk',15));                 % Morphological opening.
...
maxs = imclose(maxs, strel('disk',15));
...
[L, num1] = bwlabel(L);
chhotamota = regionprops(L, 'Area', 'PixelIdxList', 'Perimeter', 'MajorAxisLength', 'Eccentricity');
...
for i = 1:num1
%         fprintf('%d\n', chhotamota(i).Area);
        if chhotamota(i).Area <=30000 ...                                                   % put in your own criteria for the area... read the starting note.
...
%% P53 cytoplasmic annulus

PA = imdilate(L, strel('disk', 15));
PA = PA - L;                              % P53 annular region
...
overlay3 = imoverlay(A, mask1, [.8, .5, .3]);

% figure(17), imshow(overlay2), title('Mask');
% figure(19), imshow(overlay3), title('Annular');
...
pc = regionprops(PA, PF, 'Area', 'MeanIntensity');
pn = regionprops(L, PF, 'Area', 'MeanIntensity');
hstd = regionprops(L, stdfilt(PF), 'Area','MeanIntensity');
hent = regionprops(L, entropyfilt(PF), 'Area','MeanIntensity');
...
PCI = zeros(num,1);     % P53 annulus
PNI = zeros(num,1);    % P53 nuclear
PRI = zeros(num,1);
...
for j = 1:num
    ...
    idx4 = hent(j).MeanIntensity*idx2;		% k is for PCNA
    idx5 = hstd(j).MeanIntensity*idx2;
    idx6 = k(j).MeanIntensity;
    idx7 = kent(j).MeanIntensity*k(j).Area;
    idx8 = kstd(j).MeanIntensity*k(j).Area;
    idx9 = pc(j).MeanIntensity;
    idx10 = pc(j).Area;
    ...
    KI(j) = idx6*idx2;
    PNI(j) = idx3*idx2;
    PRI(j) = PNI(j)/PCI(j);			% Cytoplasmic to nuclear ratio
	  ...
	  fprintf(f,'%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n', DI(j), KI(j), idx6, idx7, idx8, PCI(j), PNI(j), PRI(j), idx4, idx5);
    % DAPI_total, PCNA_total, PCNA_mean, PCNA_entropy_texture, Ki67_stdDev_texture, p53 Cyto, p53 Nuclear, P53_Nuc2Cyto_Ratio, P53Nuc_std_texture, P53Nuc_entropy_texture
